...
5. **Capture record and batch undo.** Every confirmation creates a Capture record linking all items created from it. One action requests reversal of the whole batch through the conflict-aware Proposal and action-log mechanism in Section 11. It must not delete any item whose revision changed after apply or that acquired dependents; conflicts are shown for review.
...
Use one hierarchical `Project` entity with `kind = area | project`. An `area` has no parent. A `project` may have one area parent in V1; project-to-project nesting is deferred. Enforce cycle prevention even though V1 permits only one level. Project removal is archive/soft-delete in V1; hard deletion is rejected while dependents or audit references exist.
...
Store required integer `computed_priority_score` (0–100, default 50) separately from nullable `user_priority` (`must | should | could`). `effective_priority` is derived: the manual value wins; otherwise 70–100 = `must`, 40–69 = `should`, and 0–39 = `could`. AI recalculation may update only the computed score and must never overwrite the manual value. The interface presents the effective value and allows clearing the override.

---

## 9. Data Model

Unless a subsection says otherwise, Task, Event, Note, Person, Project, WorkSession, Conversation, and Message use the common mutable-record fields defined in Section 9.1. Capture and immutable ActionLog use their explicitly listed fields.
...
- Common fields are the mutable-record fields defined in Section 9.1. Required fields: non-empty `title`; `kind: meeting | appointment | personal | deadline | block | other`; `schedule_type: fixed | flexible`; `block_state: planned | in_progress | completed | missed_unconfirmed | cancelled` for blocks and null otherwise.
...
- When a planned block's end passes without a completion signal or linked WorkSession, set `block_state = missed_unconfirmed`. This means only “outcome not recorded”; it does not assert that the user failed to work. The task remains open and may be proposed for rescheduling.
...
- **Capture:** opaque UUID `id`, `revision`, nullable `raw_text`, nullable `redacted_text`, `source_type: typed | pasted | dictated`, `processing_status: received | redacted | proposed | processed | failed | no_ai`, `ai_excluded`, timestamps, and `raw_delete_after`. `raw_text` is required before deletion; `redacted_text` is required before an AI Proposal and null for `no_ai`. There is no `proposal_id`; Proposal has nullable `capture_id`, allowing one Capture to have multiple processing Proposals. Set `raw_delete_after` to 30 days after successful/no-AI processing; failed/unprocessed Captures are discarded or resolved and never retained more than 30 days after creation. Action logs and extracted records do not duplicate raw text.
...
- **Project:** one hierarchy table with `kind: area | project`, `status: active | completed | archived`, required `name`, nullable `parent_id`, `description`, and `importance: low | medium | high`. Areas require null parents; V1 projects may have one area parent only. Reject cycles. Archive/soft-delete is the only V1 removal operation; hard deletion is rejected while any dependent or audit reference exists.
- **Proposal, Action log:** see Section 11
- **User / UserSettings:** exactly one provisioned `User` row with opaque UUID, credential reference(s), session-revocation version, and timestamps; no registration flow. Exactly one `UserSettings` row references it and owns `current_timezone` (IANA, default `America/New_York`) plus related child rows for weekday availability, hard-unavailable windows, soft preferred windows, buffers, block lengths, and deep-work caps. Settings/constraints carry revisions and are versioned inputs to scheduler Proposals.
- **Conversation, Message:** see Section 12.5
- **Glossary entry:** `term`, `expands_to`, optional stable `entity_type` + `entity_id`; glossary membership never exempts text from the privacy guard.
- **FieldEvidence:** one row per extracted field with `proposal_operation_id`, field path, Capture start/end offsets, literal text, `confidence: high | medium | needs_confirmation`, and optional resolver metadata. Evidence spans refer to Capture text and are deleted or rendered unavailable when retained raw text expires.
...
2. Data export to JSON and iCal at any time. Data is never locked in. **Phase 3b (V1).** Export covers active and archived domain records, Proposals/action logs, Conversations/Messages, settings, and any retained Capture text; secrets, password credentials, and already-expired raw text are excluded.
...
- ProposalOperation: opaque UUID `operation_id`; parent `proposal_id`; unique non-negative `sequence`; `depends_on_operation_ids` containing only earlier operations in the same Proposal; `op: create | update | archive | restore | delete`; `entity_type`; required preallocated opaque UUID `entity_id` (including creates); nullable `expected_revision` (required for update/archive/restore/delete and null for create); schema-valid `after` payload/patch; server-captured `before` snapshot; `reason`; and field evidence/confidence references.
...
2. Validation rejects unknown IDs, invalid operation dependencies, cycles, illegal field combinations, fixed-event overlaps, hard-unavailable windows without an explicit user override, stale expected revisions, and insufficient permissions. Soft preferred/working windows may produce warnings but are not universal validation failures.
...
7. Phase 1 Inbox Proposals use `approval_policy = explicit`, `approval_tier = null`, and always require user approval. Phase 3a introduces `tiered`; Phase 1 must not implement that permission engine prematurely.
...
6. **At rest and retention:** V1 relies on hosting-provider disk/database encryption at rest and does not implement application-managed field-level keys. Raw Capture text is automatically deleted 30 days after processing, including no-AI processing; derived records retain links that show the source has expired. Provider payloads are not copied into application logs.
...
Allowed `entity_type` values in Phase 1 are `task | event | note | person | project`; reminders and waiting-for items use `task_kind`. `item_ref` values are unique only within this provider response and express extraction dependencies; the model never invents database or operation UUIDs. Existing projects and people are referenced only by candidate stable IDs that trusted code supplied to the model. Unknown names remain `unresolved_literal` or become an explicit proposed Person/Project create item requiring approval; a free-text `project`/`people` value is never silently saved. Every proposed field that derives from source text has its own evidence span and confidence.
...
**Phase 1: Foundation**
Authoritative database/data model (including Task kinds `waiting_for` and `reminder`, Capture, Person, Project hierarchy, Event blocks, revisions, Proposal, and ActionLog); projects; tasks; events; online text Inbox with OpenAI extraction, field evidence, privacy guard, mandatory confirmation, idempotency, and conflict-aware batch undo; persistence; defined single-user auth; raw-Capture expiry job. Non-sensitive iPhone keyboard/browser dictation may supply text, but AfHey accepts/stores no audio. No image/screenshot input, PWA share target, offline queue, external transcription API, general AfHey tier engine, scheduler, or calendar UI.
...
**Phase 2: Time**
Choose and record the calendar rendering library before implementation; fixed events; deterministic scheduler with constraints, explicit remaining estimates, and feasibility check; basic search; day view using the library; Today screen (fixed events, planned block Events, effective Must/Should/Could priority, Waiting For, capacity check, roll-over prompt); `missed_unconfirmed` handling and rescheduling Proposals.
