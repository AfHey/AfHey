...
## 2026-09-05 Corrections surfaced by eval-v2 (guard damper, zone adjacency, redundant items)
- Decision (privacy guard): the capitalized-pair name detector no longer treats a capitalized word followed by a weekday, or by a month and a day number, as a name ("Dentist February 30", "Retreat September 12", "Appointment Thursday"), and no longer treats a sentence-initial imperative verb + name as a name pair ("Add Neri", "Meet Olin"); after skipping a non-name first word it rescans from the second word so "Call Ada Byron" still masks "Ada Byron". A month with no day number after it ("Theresa May") is still masked.
- Decision (temporal): a zone written immediately after a temporal phrase (", UTC−05:00", " Europe/London") that the provider left outside the literal is attached deterministically; an event's end phrase resolves in the event's own zone; "one calendar day" parses like "one day"; a clock-time range filed as an event's end ("September 12" + "9am–11am") is resolved together with the start as one range.
- Decision (interpretation): two items whose title and temporal evidence spans are identical are one item (event kept over task over note, with an info warning); a note whose body is the entire capture is dropped when anything else was extracted. Prompt p6/p7 add rules for retractions, anchors, zones inside literals, one item per commitment, reminder vs action, and ranges inside the start literal.
- Rejected: leaving the month/verb false positives to the redaction preview (the preview cannot repair them — edits are re-guarded in full, so "Add Neri" was re-masked every time); asking the model alone to keep zones inside literals (deterministic code owns zones); dropping intra-capture duplicates by title similarity (identical evidence spans is the only safe signal).
- Reason: the guard's imperative-verb exemption reduces masking for a small closed word list that contains no plausible given name; every other change is deterministic salvage of what the source text already states. Evidence: with scripted correct extractions the pre-fix pipeline failed 8 checks in 5 of the 20 review cases (docs/evals/eval-v2-scripted-v2-scripted-baseline-before-fixes.md); live runs went 14/20 (p5) → 16/20 (p6) → 20/20 (p7).

## 2026-09-05 eval-v2: the review's 20 held-out cases through persisted Proposals (finding 22)
- Decision: eval-v1 (33 guarded-input cases scored on provider output) is kept as a smoke suite. eval-v2 encodes the independent review's 20 fictional cases end to end — raw capture through guard, entity resolution, provider, interpretation, and the persisted Proposal — with per-case fixtures (ambiguous first names, nested project names, an existing meeting, a known participant) and checks that read only persisted rows, Capture state, and outbound payloads intercepted at the transport. Checks are tallied per category with denominators; extra operations count as false positives; acceptance tolerates no critical failure (authoritative dates, privacy, authorization), no provider error, at most 10% extra operations, and requires 90% of all checks. A scripted-extraction variant (the correct provider output for each guarded payload) runs in CI so the deterministic pipeline is gated independently of the model; `npm run eval:pipeline` runs the pinned model and writes docs/evals/.
- Deviations from the review's wording, each stated in the report: case 14 gained a title ("Design review"); case 9 simulates the worst preview edit (the phone number re-inserted); case 10 exercises both orders of the keep-private race; case 18 accepts "explicitly unresolved" because events are not offered as anchors in Phase 1.
- Result: scripted run 20/20 cases, 77/77 checks (after the corrections in the preceding entry; 8 checks in 5 cases failed before them). Live runs on gpt-5.4-mini-2026-03-17: p5 14/20 cases (61/75 checks, 24/27 critical), p6 16/20 (67/75, 24/27), p7 20/20 (77/77, 27/27, 0 extra operations, 0 provider errors). Model output varied between runs on the same prompt (cases 2 and 15 passed under p5 and failed under p6), so one passing run is evidence, not proof; rerun on every prompt or model change.
...
- Step 1 spike result (2026-09-05): **pinned `@fullcalendar/react` 7.1.0 with `temporal-polyfill` 1.0.4; no fallback needed.** Proven in Next 16 by e2e: zone-correct rendering of seeded fixed Events, mouse drag and resize reporting instants in the user's zone, range selection, and a long-press touch drag on an iPhone-sized Chromium viewport; no hydration errors. Two v7 facts shaped the implementation: the grid renders client-only because Node's and the browser's `Intl` range formatting differ in invisible spacing (the server HTML failed hydration otherwise), and the theme is a plugin that must be registered (CSS alone leaves the grid unstyled and without resize handles). Resize works on desktop and touch, so the Phase 5 resize deferral is not exercised. A physical-iPhone pass stays a manual check for the product owner.
...
## 2026-09-05 Phase 2 plan approved; open items resolved
- Decision (product owner, on approving docs/phase-2-plan.md): (1) employment hours are `availability_window.kind = job` rows governed by `scheduler_preferences.job_time_policy`, and "work-related" means the task's area has `project.domain = work`; (2) a manual drag of a block onto a fixed Event or another block is allowed with a visible warning, while the scheduler itself never creates overlaps; (3) the roll-over prompt's Reschedule and Postpone produce Proposals and Move to Backlog is a direct edit; (4) defaults are Monday week start, 7-day planning horizon, 25/120-minute block bounds, 10-minute buffer, 240-minute daily deep-work cap; (5) if touch resize proves costly in the Step 1 spike, resize is deferred to Phase 5 and recorded, while day/week views, drag, and lock stay; (6) `plan_day`, `plan_week`, `schedule_task`, `reschedule_day` ship in Phase 2 and `lighten_day`, `make_room_for`, `postpone_low_priority_tasks` ship in Phase 3a on the same engine.
- Rejected: a flag on job Events for work time (the job is availability, not an Event; meetings inside it remain fixed Events); refusing manual overlaps (overlapping real commitments is the user's right, spec §11.2 rule 2).
- Reason: each answer picks the representation that keeps the scheduler's inputs explicit settings data rather than inferred from Event titles.

## 2026-09-05 Phase 2 migration: scheduler constraint tables, Project.domain, search columns
- Decision: one additive migration creates `availability_window` (ISO weekday, local start/end, `kind: general | job`), `protected_window` (`recurrence: weekly | once` with weekday or date), `preferred_window` (optional weekday and work type), and `scheduler_preferences` (one row per UserSettings: min/max block, buffer, daily deep-work cap, `job_time_policy`, planning horizon — defaults 25/120/10/240/`work_related_only`/7). It adds nullable `project.domain: work | personal` (CHECK: areas only; projects inherit their area's domain) and PostgreSQL-generated `search_vector` columns with GIN indexes on task, project, note, and event. Rules the database can state are CHECK constraints (start < end, weekday 1–7, recurrence exactly-when, min ≤ max) and are mirrored by Zod/invariant validators. Provisioning creates the preferences row; windows are settings rows edited directly (revision-guarded) with plain deletion, not archive.
- Rejected: storing constraints as JSON on UserSettings (no per-row revisions or CHECKs); a `domain` on every project (two sources of truth for one fact); application-maintained search columns (drift risk).
- Reason: the scheduler's inputs must be explicit, validated, versioned settings data, and search must not depend on a write path remembering to update it.

## 2026-09-05 Scheduler engine rules (Phase 2 Steps 3–6)
- Decision (timeline): availability, protected, and preferred windows are local wall-clock rules converted per day in the user's zone; a nonexistent local time resolves to the DST transition instant (a gap never lengthens a window) and an ambiguous one takes its first occurrence; busy items are padded by the buffer on both sides; free time snaps to a 5-minute grid. **All-day Events do not consume scheduler time** — a whole-day absence is a protected window (one-off or weekly), so a birthday or a delivery window on the calendar does not silently erase a day's capacity.
- Decision (priority): `computed_priority_score` = 30 + urgency (35 overdue … 3 beyond two weeks) + 5 for a hard deadline + workload pressure (0/4/8) + importance (project, else area; low 3 / medium 10 / high 20 / unknown 8) + age (0/3/6/10) − 10 for `waiting_for`, clamped to 0–100; recomputed before every plan and by the maintenance job; `user_priority` is never written.
- Decision (placement): tasks are ordered by effective band, then slack to the deadline (eligible free time up to the deadline, bounded at 14 days, minus remaining work), then score, age, id. Candidates are the start of each free interval and every applicable preferred-window start inside it; a candidate scores preferred-window coverage (0–100), minus a small earliness penalty, plus 10 when it holds the whole remaining work (fewer splits). Splittable tasks respect min/max block lengths and never leave a remainder shorter than the minimum; a non-splittable task needs one slot for its whole remaining estimate regardless of the maximum. Deep-work minutes per local day are capped, counting kept blocks. A hard deadline forbids later placement; a soft one allows it with the stated reason. Job windows admit a task only under the job-time policy and the task's area domain.
- Decision (diff and lifecycle): only flexible, unlocked, `planned` blocks of in-scope tasks are re-plannable; fixed, locked, in-progress, and completed blocks are kept and busy; kept future blocks reduce a task's remaining work. Re-plannable blocks are paired with new placements per task in start order — unchanged pairs produce no operation, changed pairs a move, surplus blocks a cancel, surplus placements a create. A `missed_unconfirmed` block is never moved: `reschedule_day` cancels it (it stays on record as missed) and places the work in a new block; `plan_day` leaves missed blocks to the Step 7 prompt.
- Decision (Proposals): every operation returns a Proposal with `origin = scheduler`, explicit approval, `expires_at` at the end of the planned range, and a template reason per operation. Rule §11.2.2 is enforced twice: at build time a scheduler Proposal is rejected if any resulting block overlaps a fixed Event, a live block, another block in the Proposal, or a protected window (unless `allow_protected_override`); at apply time each block create or move is rechecked inside the transaction against events added or moved since, and conflicts rather than double-books. Event payloads now accept `kind = block` with `task_id` and `block_state`; extraction still cannot produce blocks because interpretation maps event kinds from a list without it.
- Rejected: moving a missed block forward (hides that it was missed); treating all-day Events as busy (too coarse); an unbounded slack horizon (cost) and slack measured only inside the planning range (a task due next week would tie with one due tonight); silently letting a non-splittable task exceed the maximum block length without a stated rule (it is a rule: the maximum governs splitting only).
- Reason: each rule is deterministic, explainable in a one-line reason on the operation, and testable in isolation (suites G, H, I, K).
...
## 2026-09-05 Block lifecycle, calendar, and Today rules (Phase 2 Steps 7–9)
- Decision (block lifecycle): drag, resize, lock, mark fixed, unschedule, and "Mark work done" are direct revision-guarded mutations, not Proposals (spec §12 rule 13: manual single-entity edits). A manual move that lands on a fixed Event, another block, or a protected window is saved and returns warnings that the UI shows beside the result; the scheduler alone is forbidden to overlap. Moving a `missed_unconfirmed` block returns it to `planned`. Stopping a focus session settles the block by the clock: before the block's end it is `planned` again, after the end it is `completed`. "Mark work done" writes one WorkSession spanning the block, and repeating it never writes a second. Only one WorkSession can be active; starting another is refused rather than silently stopping the first.
- Decision (calendar): the calendar is a view over Events and blocks with FullCalendar drawing the grid; deadlines are all-day markers derived from Tasks at render time and are never rows. A scheduler run is reviewed as a diff of operations with per-operation keep boxes; unticking operations revises the Proposal (same origin, new operations) before approval, and one "Undo this plan" reverts the whole apply through the existing conflict-aware undo Proposal. Missed blocks are counted in a banner on load, and a block's actions live in a panel opened by tapping it, so touch and mouse share one path.
- Decision (Today): the "+ Add" required by spec §10.3 is the app frame's existing link, which opens the Inbox composer focused (`/inbox?focus=1`) from Today as from every screen; Today has no second capture form because the Inbox is the single extraction entry point. The capacity line counts eligible free minutes from now to the end of the day (fixed Events and fixed, locked, or in-progress blocks are busy, padded by the buffer) against planned block minutes still ahead. The roll-over section lists yesterday's `missed_unconfirmed` blocks: Reschedule into today and Postpone produce `roll_over` Proposals, Move to backlog is a direct edit that backlogs the task and unschedules the block. The root path redirects to Today; the phone tab bar shows Today, Calendar, Inbox, Tasks, and Settings, with Events, Projects, Notes, and People reachable from the desktop navigation. Non-block Events carry a NULL block state, so every "not cancelled" query spells out `block_state IS NULL OR block_state <> 'cancelled'`; a bare negation would drop all plain Events (found in Step 8's e2e run).
- Rejected: routing manual moves through Proposals (the spec reserves Proposals for AI and scheduler changes); refusing overlaps on manual moves (rejected on approving the plan); a separate Quick Add form on Today (duplicated capture path); moving the `missed_unconfirmed` block forward on roll-over (it is cancelled and the work re-placed, so the miss stays on record).
- Reason: keeps direct edits cheap and Proposals meaningful, and gives every automatic change a review surface and a single revert.

## 2026-09-05 LAN access to the development server
- Finding: signing in from the phone at `http://100.110.160.140:3000` accepted the password but stayed on `/login`. Neither the cookie nor CSRF was the cause: the session cookie is `Secure` only under `NODE_ENV=production`, and the same-origin check compares the Origin host with the Host header, which match on a direct LAN request. The dev server's log showed Next 16 returning 403 for `/_next/static/*` and `/_next/hmr` to the host `100.110.160.140` (its default cross-origin guard for dev-only resources), so the login form never hydrated and submitting it reloaded the page.
- Decision: `next.config.ts` sets `allowedDevOrigins` from `AFHEY_DEV_ORIGINS` (comma-separated hostnames, parsed by `src/lib/dev-origins.ts`), documented in `.env.example`. Next applies the list in development only; `next start` ignores it. The cookie policy is unchanged: `HttpOnly` and `SameSite=Lax` always, `Secure` in production, so a production deployment still requires HTTPS. The e2e suite proves the path with a login over plain HTTP from the non-localhost host `lan.test` (Chromium host-resolver mapping to the loopback e2e server) and asserts the cookie arrives without `Secure`.
- Decision (test isolation): the Playwright web server builds into `.next-e2e` (`AFHEY_DIST_DIR`), because Next 16 refuses a second `next dev` while another holds the lock in `.next/dev`; the suite now runs beside a manually started dev server instead of requiring it to be stopped.
- Rejected: dropping `Secure` in production or keying it on the request scheme (a downgrade path); hard-coding the Tailscale address in the config (machine-specific and not a secret, so it belongs in the local `.env`); allowing `*` (the guard exists to keep other LAN hosts from loading dev internals).
- Reason: the fix targets the actual blocker, keeps production strict, and stays out of the committed code except as an opt-in list.

## 2026-09-06 Phone feedback before Steps 10–12
- Decision (empty plans explain themselves): a scheduler run reports `free_minutes` in the range from now, the number of eligible tasks, and how many were already covered by kept blocks, alongside the unplaced and estimate-required lists. The review headline is composed from these (`src/components/plan-headline.ts`): no eligible tasks; no free time left in the day or week (availability, protected windows, and fixed Events leave nothing); tasks that cannot fit; tasks needing an estimate; tasks already planned. "Nothing to change" appears only when none of these applies. Today shows the same review instead of a separate message.
- Decision (estimate inline): a task flagged `estimate_required` on Today carries a minutes field and "Set estimate", a direct edit that sets `remaining_estimate_minutes` and, as the first estimate, `estimated_duration_minutes` to the same value. Rejected: sending the user to the task form (breaks the plan-review loop on a phone).
- Decision (calendar palette): FullCalendar's Breezy palette is mapped onto the design tokens inside `.afhey-calendar` (surface, ink, line, brass, danger) and its dark neutrals are set under `prefers-color-scheme: dark`, because the theme only switches to dark through a `data-color-scheme` attribute the app does not set. Event text is always full ink (the theme's half-mixed colour failed contrast on light backgrounds); blocks carry a solid border in their state colour with a 4px start edge and a 26% tint; block state colours are tokens (`--block-planned/active/missed/done`) with light and dark values, referenced by the view-model rather than hex literals.
- Decision (resize): blocks and Events resize from the top edge as well as the bottom (`eventResizableFromStart`); both edges save through the same move mutation.
- Decision (development data): `npm run db:unseed` removes the fictional seed from the database at `DATABASE_URL` by the seed's fixed ids only (`prisma/seed-ids.ts`): projects, people and aliases, glossary entries, tasks, Events, notes, and any blocks or WorkSessions of seeded tasks; user-created rows keep their data and lose only a link to a seeded project. The Work and Personal areas, availability, protected and preferred windows, and scheduler preferences stay: they are the user's settings structure, not fictional content. Run once against `afhey_dev` on 2026-09-06; test databases are still seeded by the e2e and Vitest setups. Rejected: a one-off SQL delete (not repeatable, no record); wiping `afhey_dev` (the owner's own captures, tasks, and Proposals live there).
- Reason: each item came from the product owner's first phone pass and is a fix to Phase 2 behaviour, not new scope.

## 2026-09-06 Search rules (Phase 2 Step 10)
- Decision: search runs entirely in PostgreSQL over the generated `search_vector` columns (`websearch_to_tsquery`, `ts_rank`, `ts_headline`) across Tasks, Projects, Notes, and Events, with filters for type, project, task status, task bucket, and a due window. Date phrases in the query ("things due Friday", "ledger by Friday", "Sept 12–14") are detected and resolved by the deterministic temporal resolver from interpretation; "due X" and "on X" mean that day, "by/before X" means today through X, "after X" means the following 30 days, and an ambiguous phrase widens to both readings with a visible note. Once a due window is present, filler words (things, items, tasks, show, all…) are dropped and the search is filter-only over Tasks and Events, the two types that carry dates. Archived rows and work blocks are never returned (a block's task is the record). `ai_excluded` records are searchable: search is local and no text leaves the machine. Highlight markers are control characters rendered by the page, never HTML from record text. The navigation search box and the Search page are plain GET forms with shareable URLs, so they work before hydration and on a phone.
